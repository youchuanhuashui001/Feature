...
## 引脚复用有 3 个脚，但是插上去的发送管只有两个脚，这个管也可以用于接收，那么这 3 个脚是怎么处理的？
- IR_TX
- IR_RX
- IR_EN

## 使用 gx_ir send_repeat 1 1 间隔大约 95ms 左右会继续发下一个 repeat 码，但是按键是会间隔 108ms 产生一个 repeat 码？
...
## 为什么带载波自由模式发送时需要用机顶盒的板子来接收，不能用 apus 接收？

^de395a


## 我们用的这个红外一体头，由于发送的时候会调制信号，接收的时候不会解调信号，但是用这个器件，发送很好用，但是接收不好用，如果事业部要用接收的时候怎么办？
- 如果要换成接收时自带解调的器件，那么就不需要驱动来解调了吧？
	- `rx.demode_en = 0` 关掉接收解调
- 如果换成自带解调的器件，应该接收能力很强吧，但是发送能力又很弱了，这样就很难同时兼容发送和接收吧？


## gx_hal_ir_init 接口中何时会使能 internal_ana?
- 模拟发送和模拟接收，还有非模拟发送和非模拟接收吗，也就是数字部分？概念不太理解
- 芯片阶段才会使用模拟功能吗？
