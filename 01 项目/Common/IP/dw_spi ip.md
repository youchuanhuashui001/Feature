...
# Baud 寄存器
- 当 SPI 工作在 Slave 模式，此寄存器配置无效。
![[Pasted image 20250628133154.png]]
...
### 注意事项：
- 编程了地址和指令的情况下：需要注意指令和地址占用 FIFO 的情况，手册描述如下：
![[Pasted image 20250429164819.png]]
...
## Sample_delay
- 只有在 Master 时需要，做 slave 的时候不需要
![[Pasted image 20250702142746.png]]
...
## Burst/Single 切换
当 DMA 控制器中编程的块大小 (BLOCK_TS) 不是突发事务长度 (SRC_MSIZE 或 DEST_MSIZE) 的整数倍时，在完成整数倍的突发事务后，可能还会剩余一些数据需要传输 。此时，`dma_tx_single` (当发送 FIFO 至少有一个空闲条目时激活) 和 `dma_rx_single` (当接收 FIFO 至少有一个有效数据条目时激活) 信号会通知 DMA 控制器可以执行单个数据项的传输，以完成整个数据块的传输。
![[Pasted image 20250509172932.png]]
...
# Bug:
## Bug1：Apus nre flash xip 在某些情况下访问出现卡死情况
...
## Bug2：单线模式会出现 Receive FIFO Overflow
- 问题号： https://git.nationalchip.com/redmine/issues/341673
...
# 问题：
## 问题 1：DTR 模式下只有单个相位 0可用
### 问题现象
virgo nre 测试时增加了 dtr 模式，非 dtr 模式时测试有多个相位，dtr 模式无论是 4 分频、8 分频、16 分频都只有一个相位 0 可用

### 问题原因
由于 dtr 模式采样是双边沿采样，在 flash_spi 模块工作频率低的时候窗口右边是 0，左边到了负值；在 flash_spi 模块工作频率高的时候窗口左右都有值。


### 解决办法
芯片出了一个 flash_spi 模块频率为 48MHz 的 bit，测试 dtr 模式有两个相位可用 (0,1)
...
#### 1. SPI Slave发送数据（TX FIFO）：

- ​**​功能​**​：TX FIFO（发送缓冲区）作为临时存储空间，用于存放待发送的数据。
- ​**​工作流程​**​：
    - ​**​预填数据​**​：在SPI Slave启动前，需预先将待发送数据写入TX FIFO。
    - ​**​主机触发发送​**​：当主机提供时钟信号（CLK）时，Slave会从TX FIFO中依次取出数据发送。
    - ​**​空FIFO处理​**​：若主机CLK到来时TX FIFO为空（未填新数据），Slave会重复发送上一次的数据（保留旧值）。
- ​**​关键点​**​：
    - 必须提前填充TX FIFO，否则会重复发送旧数据。
    - 数据按先进先出（FIFO）顺序发送。

#### 2. SPI Slave接收数据（RX FIFO）：

- ​**​功能​**​：RX FIFO（接收缓冲区）作为临时存储空间，用于暂存从主机接收的数据。
- ​**​工作流程​**​：
    - ​**​数据接收​**​：主机通过CLK发送数据时，Slave将数据逐位存入RX FIFO。
    - ​**​溢出风险​**​：若RX FIFO已满（例如容量为8字节）且未被及时读取，新数据会导致溢出（数据丢失）。
    - ​**​及时读取​**​：在FIFO填满前读取数据，可确保后续数据正常接收。
- ​**​关键点​**​：
    - 需及时清空RX FIFO，避免溢出。
    - 数据按接收顺序存储在FIFO中，读取时也遵循先进先出。

---

### 统一总结（SPI Slave的FIFO机制）：

1. ​**​发送方向（TX FIFO）​**​：
    
    - 写数据：用户提前写入待发送数据。
    - 读数据：主机CLK触发Slave自动从TX FIFO读取并发送。
    - 空值处理：TX FIFO为空时，自动重复最后一次发送的数据。
2. ​**​接收方向（RX FIFO）​**​：
    
    - 写数据：主机CLK将接收的数据写入RX FIFO。
    - 读数据：用户需及时读取RX FIFO数据。
    - 溢出条件：未及时读取导致FIFO满时，新数据丢失。
3. ​**​共同特点​**​：
    
    - FIFO作为缓冲，协调主机（CLK驱动）与Slave（用户控制）的速度差异。
    - 需用户主动管理FIFO的填充（TX）和清空（RX），避免数据错误或丢失。

---

### 注意事项：

- ​**​FIFO大小​**​：文中提到RX FIFO容量为8字节，需根据实际硬件确认。
- ​**​时序要求​**​：用户操作（填充/读取）需快于主机CLK速率，尤其在高速SPI模式下。
- ​**​错误处理​**​：溢出或重复发送旧数据可能引发通信错误，需通过状态寄存器监控FIFO状态。


### ​**​SPI Slave 接收数据（RX FIFO）补充说明​**​

在 SPI Slave 模式下，​**​RX FIFO（接收缓冲区）​**​ 的工作机制如下：

#### ​**​1. 数据接收流程​**​

- ​**​Master 发送数据​**​：主机（Master）通过 CLK 信号逐位发送数据，Slave 在 CLK 边沿采样并存入 RX FIFO。
- ​**​FIFO 存储规则​**​：
    - ​**​如果 RX FIFO 未满（有剩余空间）​**​：新数据会正常存入 FIFO，不会丢失。
    - ​**​如果 RX FIFO 已满​**​：新数据会被丢弃（溢出），可能导致通信错误。

#### ​**​2. 关键点​**​

- ​**​用户必须及时读取 RX FIFO​**​，避免因 FIFO 满导致数据丢失。
- ​**​FIFO 深度（容量）​**​ 决定了能暂存多少数据（例如 8 字节），需根据通信速率调整读取速度。
- ​**​硬件可能提供状态标志​**​（如 `RX_EMPTY`、`RX_FULL`），用于判断 FIFO 状态，优化读取时机。

#### ​**​3. 典型场景​**​

|情况|RX FIFO 状态|数据是否存入|风险|
|---|---|---|---|
|用户读取速度快于 Master 发送速度|FIFO 始终有空闲空间|✅ 正常存储|无|
|用户未及时读取，FIFO 填满|FIFO 满|❌ 新数据丢弃|数据丢失|
|用户读取部分数据，FIFO 未满|FIFO 有空闲空间|✅ 正常存储|无|

#### ​**​4. 如何避免数据丢失？​**​

- ​**​方法 1​**​：轮询或中断方式监控 FIFO 状态，及时读取数据。
- ​**​方法 2​**​：提高 Slave 读取速度（如 DMA 传输），确保 FIFO 不堵塞。
- ​**​方法 3​**​：选择足够深的 FIFO（如 16/32 字节），适应高速通信。

---

### ​**​总结​**​

- ​**​RX FIFO 仅在满时才会丢弃数据​**​，只要有空闲空间，Master 发来的数据均会正常存储。
- ​**​用户需保证及时读取​**​，避免溢出问题，否则可能丢失关键数据。
- ​**​硬件 FIFO 深度和读取策略​**​ 直接影响通信可靠性，需根据实际应用优化。