...
- 支持自动流控
- 支持可编程的 THRE 中断模式
- 不支持 FIFO 可访问模式 (测试时使用)
- 支持 DMA
- 支持小数波特率，并且小数分频位宽为 4
- 支持影子寄存器，减少软件开销

注：对接 dma 时，dma 的操作 uart 寄存器时位宽需要配置为 8bit，因为 uart 的 fifo 位宽为 8bit。

接收 fifo 溢出位：`LSR[1].OE`
- 当为 fifo 模式时，如果 fifo 已满，并且一个新的字符在接收器上。fifo 中的数据会被保留，而接收移位寄存器中的数据会丢失。读取 LSR 或 RBR 会清除 OE 位(LSR_STATUS_CLEAR = 0)。

奇偶校验：
- 开启奇校验，校验位 + 数据位中的 1 的总和为奇数
- 开启偶校验，校验位 + 数据位中的 1 的总和为偶数
- 示波器或逻辑分析仪查看开启就校验后的校验位数据是否正确
- 需要故意翻转数据，或者故意翻转校验位来营造奇偶校验出错的情况，但驱动无法修改数据和校验位，无法营造出奇偶校验出错的情况。
...
该模块的最初设计主要是由于 uart 的时钟不稳，容易上下波动，并且波动的还很大。当上位机连续发送 0xef 时，模块能够通过 uart brc 模块来计算出 5 个 bit 数据的传输耗时时间，再根据这些时间来计算出自己的 div 和 dlf 来与对方匹配上。或者根据这些时间以及对方发送 0xef 的波特率来计算出自己的模块频率。
目前 UART 的时钟都是来自晶振或者时钟树，都是稳定的。因此该模块也不怎么会使用到。
