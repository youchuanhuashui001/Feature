...
# 链接
链接脚本用来指定编译时的一些选项，使程序能够按照开发者的想法来进行指定的排布。
- 链接地址：链接脚本中指定，理论上程序运行时所处的地址。在编译的时候代码都会以这个地址来翻译位置有关码，即一些变量的地址都是用的绝对地址，有些函数用的是偏移地址。
- 加载地址：程序运行时，实际所在的地址。

gxloader：链接地址 = 加载地址
- 链接地址：stage1 在 sram，stage2 在 ddr
- 加载地址：Stage1 中 copy_from_flash 代码就是把 stage2 的代码从 flash 的某个地址拷贝到 stage2 所在的链接地址

uboot：
使用参数 `pie` 会生成 `.rel.dyn` 段，里面会存放位置有关代码的符号表
一开始加载 uboot 代码到加载地址。然后加载之后的代码会把自己重新再拷贝到 ddr 的尾部，但是设计到变量是位置相关码，所以需要依赖符号表将变量相关的代码也都拷贝过去

`.rel.dyn` 段中存放着全局非指针变量的Label的地址和全局指针变量的地址。
.rel. dyn 段是在链接器 ld 链接的命令中加入-pie 选项生成的，表示生成与位置无关的代码，. rel. dyn 存储的格式是 2 个 4 字节空间为一组，高 4 字节存放固定的值 0x17（可以用于校验），低 4 字节存放 Label 的地址或者全局指针变量的地址。Label 是指，ld 加入了-pie 选项后，会在调用全局变量的函数末尾生成一个空间，存放全局变量的绝对地址。函数访问全局变量的时候是先读取 Label 处存放的绝对地址，然后根据绝对地址找到全局变量。

根据. rel. dyn 段的记录，对目标地址中的 Label 进行操作，. rel. dyn 段记录的是代码中所有的 Label 的地址（源地址），我们要修改目的地址处的所有的 Label 值，所以读取. rel. dyn 中的数据，加上偏移（r4 寄存器保存的偏移）得到的就是目的地址处的 Label 的地址，此时目的地址处的 Label 处保存的值，其实是源地址中全局变量的地址，将目的地址处的 Label 中保存的值加上偏移，这样，目的地址处的 Label 中保存的值就是目的地址中全区变量的地址了。


重定位会把整个 uboot 的代码全部都重定位到 ddr 的尾部，包括 `start.S`
- 拷贝的过程会根据 `.rel.dyn` 段的信息来处理位置相关码
