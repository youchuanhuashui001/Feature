...
*   完成了 [[timer ip]]、[[rtc ip]]、[[gpio ip]] 的熟悉。
*   正在熟悉 [[dw_i2c ip]]。
*   [[dw_wdt ip]]：开始熟悉。
*   [[dw_uart fpga 验证]]：测试用例全部跑通，但需要考虑如何构造更全面的测试用例。
*   [[dw_wdt fpga 验证]]：测试用例全部过了一遍。
*   [[dw_rtc fpga 验证]]：测试用例全部过了一遍。

**Fornax 相关：**

*   [[fornax 验证 GT25Q16B 能否超频到 120MHz]]: 使用 240Mhz 的时钟频率测试。
*   Fornax 加的 GT25Q16B 需要放到 RobotOS 支持列表中。
