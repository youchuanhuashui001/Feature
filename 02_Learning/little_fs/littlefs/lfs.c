...
static int lfs_dir_splittingcompact(lfs_t *lfs, lfs_mdir_t *dir,
    ...
    while (true) {
        ...
        if (err) {
            ...
            // performance
            LFS_WARN("Unable to split {0x%x, 0x%x}",
                    dir->pair[0], dir->pair[1]);
            break;
        } else {
            end = split;
    ...
    if (lfs_dir_needsrelocation(lfs, dir)
        ...
        if (lfs->block_count - size > lfs->block_count/8) {
            LFS_DEBUG("Expanding superblock at rev %d", dir->rev);
            int err = lfs_dir_split(lfs, dir, attrs, attrcount,
                    source, begin, end);
            if (err && err != LFS_ERR_NOSPC) {
                return err;
...
static int lfs_migrate_(lfs_t *lfs, const struct lfs_config *cfg) {
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail)) {
            ...
            LFS_DEBUG("Migrating {0x%x, 0x%x} -> {0x%x, 0x%x}",
...
int lfs_fs_grow(lfs_t *lfs, lfs_size_t block_count) {
    ...
    LFS_TRACE("lfs_fs_grow(%p, %d"")", (void*)lfs, block_count);
