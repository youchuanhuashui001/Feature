...
#ifdef MY_TEST_AXI_DEBUG
	pr_info("chan_reg:0x%x\n", (u32)chan->chan_regs);
	pr_info("axi_chan_config_write cfg_lo:0x%x cfg_hi:0x%x\n", axi_chan_ioread32(chan, CH_CFG_L), axi_chan_ioread32(chan, CH_CFG_H));
#endif
}
...
#ifdef MY_TEST_AXI_DEBUG
	pr_info("aix_dma_enable DMAC_CFG:0x%x\n", axi_dma_ioread32(chip, DMAC_CFG));
#endif
}
...
#ifdef MY_TEST_AXI_DEBUG
	pr_info("axi_chan_irq_set: CH_INTSTATUS_ENA:0x%x\n", axi_chan_ioread32(chan, CH_INTSTATUS_ENA));
#endif
}
...
#ifdef MY_TEST_AXI_DEBUG
	pr_info("axi_chan_irq_sig_set: CH_INTSIGNAL_ENA:0x%x\n", axi_chan_ioread32(chan, CH_INTSIGNAL_ENA));
#endif
}
...
#ifdef MY_TEST_AXI_DEBUG
	pr_info("axi_chan_enable : DMAC_CHEN:0x%x\n", axi_dma_ioread32(chan->chip, DMAC_CHEN));
#endif
}
...
static void axi_chan_start_first_queued(struct axi_dma_chan *chan)
{
	...
	dev_info(chan2dev(chan), "%s: started %u\n", axi_chan_name(chan),
		vd->tx.cookie);
...
static int dma_chan_alloc_chan_resources(struct dma_chan *dchan)
{
	...
	dev_info(dchan2dev(dchan), "%s: allocating\n", axi_chan_name(chan));
...
static void set_desc_for_master(struct axi_dma_hw_desc *desc, enum dma_transfer_direction direction)
{
	u32 val;
	...
	if (direction == DMA_MEM_TO_DEV) {
		...
		pr_err("err set_desc_for_master !\n");
	}
...
//	set_desc_src_master(hw_desc);
	set_desc_for_master(hw_desc, chan->direction);
			      ...
			      unsigned long flags, void *context)
{
	...
	pr_info("dw_axi_dma_chan_prep_slave_sg paramter\n");
	pr_info("sgl:       0x%x\n", (u32)sgl);
	pr_info("sg_len:    0x%d\n", sg_len);
	pr_info("direction: %s\n", (direction == DMA_MEM_TO_DEV) ? "m2p" : "p2m");
	pr_info("flags:     %d\n", flags);
...
#ifdef MY_TEST_AXI_DEBUG 
	int test_tmp_sgs = 0;
	struct axi_dma_hw_desc *tmp_desc;
	for (test_tmp_sgs = 0; test_tmp_sgs < num_sgs; test_tmp_sgs++) {
		tmp_desc = &desc->hw_desc[test_tmp_sgs];
		pr_info("!!!!!!!! hw_desc:%d   !!!!!!!!!", test_tmp_sgs);
		pr_info("SAR: 0x%llx\n",            le64_to_cpu(tmp_desc->lli->sar));
		pr_info("DAR: 0x%llx\n",            le64_to_cpu(tmp_desc->lli->dar));
		pr_info("Block TS (Low): 0x%x\n",     le32_to_cpu(tmp_desc->lli->block_ts_lo));
		pr_info("Block TS (High): 0x%x\n",    le32_to_cpu(tmp_desc->lli->block_ts_hi));
		pr_info("LLP: 0x%llx\n",            le64_to_cpu(tmp_desc->lli->llp));
		pr_info("Control (Low): 0x%x\n",      le32_to_cpu(tmp_desc->lli->ctl_lo));
		pr_info("Control (High): 0x%x\n",     le32_to_cpu(tmp_desc->lli->ctl_hi));
		pr_info("Source Status: 0x%x\n",      le32_to_cpu(tmp_desc->lli->sstat));
		pr_info("Destination Status: 0x%x\n", le32_to_cpu(tmp_desc->lli->dstat));
		pr_info("Status (Low): 0x%x\n",       le32_to_cpu(tmp_desc->lli->status_lo));
		pr_info("Status (High): 0x%x\n",      le32_to_cpu(tmp_desc->lli->status_hi));
		pr_info("Reserved (Low): 0x%x\n",     le32_to_cpu(tmp_desc->lli->reserved_lo));
		pr_info("Reserved (High): 0x%x\n",    le32_to_cpu(tmp_desc->lli->reserved_hi));
	}
...
#ifdef MY_TEST_AXI_DEBUG
	pr_info("dma_transfer_direction:%s\n", (config->direction == DMA_MEM_TO_DEV) ? "DMA_MEM_TO_DEV" : "DMA_DEV_TO_MEM");
	pr_info("src_addr :       0x%x\n", config->src_addr);
	pr_info("dst_addr :       0x%x\n", config->dst_addr);
	pr_info("src_addr_width : 0x%x\n", config->src_addr_width);
	pr_info("dst_addr_width : 0x%x\n", config->dst_addr_width);
	pr_info("src_maxburst:    0x%x\n", config->src_maxburst);
	pr_info("dst_maxburst:    0x%x\n", config->dst_maxburst);
	pr_info("device_fc:       0x%d\n", config->device_fc);
	pr_info("slave_id:        0x%d\n", config->slave_id);
#endif
...
static irqreturn_t dw_axi_dma_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < dw->hdata->nr_channels; i++) {
		...
		dev_info(chip->dev, "%s %u IRQ status: 0x%08x\n",
					    ...
					    struct of_dma *ofdma)
{
	...
	if (!dchan) {
		dev_err(chan2dev(chan), "no dchan\n");
		return NULL;
	}

	chan = dchan_to_axi_dma_chan(dchan);
//	chan->hw_handshake_num = dma_spec->args[0];
	chan->hw_handshake_num = 20;
	return dchan;
}
