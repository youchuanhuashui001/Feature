...
### 提供给 SPI 设备相关的 API 与数据结构

- 挂载在 SPI 总线下的设备和驱动，结构分别为 `spi_device、spi_driver`，通过 `SPI 总线` 的匹配规则进行匹配，匹配成功后执行 `spi_driver->probe` 

1. API

   - 注册 SPI 设备到 SPI 总线：
     - 设备信息通过`struct spi_board_info` 的方式进行描述，调用 `spi_register_board_info` 来注册 SPI 设备
     - 设备信息通过 `设备树` 的方式进行描述，调用 `of_register_spi_device` 来注册 SPI 设备
     - 设备信息通过`acpi` 的方式进行描述，调用 `acpi_register_spi_device` 来注册 SPI 设备
   - 注册 SPI 驱动到 SPI 总线：配置 `spi_driver` 结构然后调用 `register_spi_driver` 进行注册 SPI 驱动

   ```c
   /* drivers/spi/spi.c */
   
   /* 用于将设备挂载到 SPI 总线下，并注册设备 */
   int spi_register_board_info(struct spi_board_info const *info, unsigned n)
   {
   	struct boardinfo *bi;
   	int i;
   ...
   /* 控制器的 bus_num 和 设备的 bus_num 进行匹配，匹配成功则创建设备 实则是 spi_device 结构的一个设备 */
   static void spi_match_master_to_boardinfo(struct spi_master *master,
   				struct spi_board_info *bi)
   {
   	struct spi_device *dev;
   ...
   /* 执行 SPI 控制器的 setup 函数，再添加 SPI 设备 */
   int spi_add_device(struct spi_device *spi)
   {
   	static DEFINE_MUTEX(spi_add_lock);
   	struct spi_master *master = spi->master;
   ...
   /* 用于将驱动挂载到 SPI 总线下，并提供 驱动和设备 匹配成功后执行的 probe 函数 实则是一个 spi_driver 的一个驱动 */
...
## 控制器驱动层：drivers/spi/spi-dw-gx-v2.c

​		控制器驱动即 SPI 控制器，负责最底层的数据收发工作，为了完成数据的收发工作，控制器驱动需要完成以下这些功能：

- 申请必要的硬件资源，例如中断、DMA 通道、DMA 内存缓冲区等
- 配置 SPI 控制器的工作模式和参数，使之可以和相应的设备进行正确的数据交换工作
- 向通用接口层提供接口，是的上层通过 SPI 协议通信的设备驱动可以通过 `通用接口层` 访问控制器驱动
- 配合通用接口层，完成数据消息队列的排队和处理，直到消息队列变空为止



- 控制器驱动层实现的是在 `platform 总线` 下的 `struct platform_device`和 `struct plarform_driver`，并且通过 `platform 总线` 匹配成功后执行 `struct platform_driver` 的 probe 函数。

  



### 控制器设备：`platform_device`

​		SPI 控制器遵循 Linux 的设备模型框架，一个 SPI 控制器在代码中对应一个挂载在 `platfom_bus` 下的一个 `device` 设备。所以在板级的代码中 `SPI 控制器` 定义一个 `platform_device` 结构即可。

​		有几种定义 `platform_device` 的方式：
...
- 动态定义的方式：
  ...
  static int __init my_device_init(void)
  {
      my_device = platform_device_alloc("my-device", -1);
      if (!my_device)
          return -ENOMEM;
  
      return platform_device_add(my_device);
  }
  ```
...
### 控制器驱动：`platform_driver`

​		`SPI 控制器` 是挂载在 `platform bus` 下的设备，有 `platform_device` ，则一定要有 `platform_driver` 与之进行匹配
...
- 注册 `platform_driver`：
  ...
  - 调用 `platform_register_driver` 来注册 `platform_driver`
  ...
  static int __init dw_spi_gx_driver_init(void)
  { 
  	return platform_driver_register(&(dw_spi_gx_driver) , ##__VA_ARGS__);
  } 
  module_init(dw_spi_gx_driver_init);
  static void __exit dw_spi_gx_driver_exit(void)
  {
  	platform_driver_unregister(&(dw_spi_gx_driver) , ##__VA_ARGS__);
  }
  module_exit(dw_spi_gx_driver_exit);
  ```
...
​		有了 `platform_device` 和 `platform_driver` 就可以根据 `platform` 总线的匹配方式进行匹配，匹配成功则会执行 `platform_driver` 的 probe 函数。

匹配方式：

- `driver_override` 和 `driver->name` 进行匹配
- `设备树` 方式
- `ACPI` 方式
- `id_table` 方式
- `platform_device->name` 和 `platform_driver->name` 的方式
...
### SPI 控制器注册：`spi_register_master`

​		SPI 通用接口层用 `struct spi_master` 结构来表示一个 `SPI 控制器`，使用 `spi_register_master` 函数来注册一个 `spi_master` 结构到 
...
## 通用设备层：gxmisc/drivers/spinor/linux/spinor.c

​		这一层设备是挂载在 `SPI 总线` 下的设备和驱动，匹配规则由 `SPI 总线` 规定，匹配成功后执行驱动的 probe。

## 设备

- 设备采用 `struct spi_board_info` 的形式描述，并采用 `spi.c` 提供的 `spi_register_baord_info` 接口根据描述的 设备信息 来创建设备
  - `spi_register_board_info` ：
    - 存在两个全局链表，分别是 `board_list 、 spi_master_list` 代表设备链表和控制器链表
    - 将使用 `struct spi_baord_info` 形式描述的设备信息添加到设备链表中
    - 根据 `bus_num` 匹配设备和控制器，若匹配成功则创建用 `struct spi_board_info` 描述的设备

## 驱动

- 驱动采用 `struct spi_driver`的形式描述，并采用 `spi.c` 提供的 `spi_register_dirver` 接口根据描述的 驱动信息
  - `spi_register_driver`：
    - 根据配置信息：`driver.owner/bus/probe/remove/shutdown` 调用 `driver_reigster` 进行注册驱动
    - 根据 `drv->bus 、bus->name` 进行匹配，然后在 `spi bus` 上注册驱动：`bus_add_driver`，并添加描述信息



## SPI 总线匹配

- 当设备和驱动都注册完成的时候，会触发 `SPI 总线` 的匹配函数：`spi_match_device`，有四种匹配方式：设备树、ACPI、id_table、`spi_device->modalias, spi_driver->name`
- 匹配完成后会执行 `spi_driver->probe` 函数
...
分区表是一块存放在flash中的数据，什么时候拿的这段数据

【分区信息】：通过 cmdline 的形式传递的，这些数据在 boot.c 中会重新构建一遍

````
Kernel command line: mem=68M@0x00000000 videomem=60M@0x04400000 mem_end console=ttyS0,115200 init=/init root=/dev/mtdblock3 mtdparts=m25p80:128k@0m(BOOT),128k@128k(TABLE),2880k@256k(KERNEL),2944k@3136k(ROOTFS),10304k@6080k(DATA) mtd_end
````

【cmdline】分区表存放着分区信息，在启动的过程中会读取分区表，把这些信息重新构建成 `cmdline`

【cmdline】：有两种方式传递，一种是通过 bootloader 传递给 linux kernel，一种是 linux kernel 设备树中的 `choose bootargs` 节点提供 cmdline



- 首先解析 mtd 分区信息，调用`parse_mtd_partitions` 使用 `cmdlinepart` 的方式解析，将解析出来的分区信息存放在 `struct mtd_partitions` 的结构中，拿到这些分区信息之后，把这些分区信息注册成分区表`add_mtd_partitions(&m25p->mtd, parsed.parts, parsed.nr_parts)`
  			 ...
  			 struct mtd_part_parser_data *data)
  {
  	...
  	for ( ; *types; types++) {
  		pr_debug("%s: parsing partitions %s\n", master->name, *types);
  		parser = mtd_part_parser_get(*types);   /* 获取解析方式，采用 cmdlinepart 的方式 */
  		if (!parser && !request_module("%s", *types))
  			parser = mtd_part_parser_get(*types);
  		pr_debug("%s: got parser %s\n", master->name,
  			 parser ? parser->name : NULL);
  		if (!parser)
  			continue;
  		ret = (*parser->parse_fn)(master, &pparts->parts, data); /* 执行 cmdlinepart->parse_fn 解析分区信息 */
  		pr_debug("%s: parser %s: %i\n",
  			 master->name, parser->name, ret);
  		if (ret > 0) {
  			printk(KERN_NOTICE "%d %s partitions found on MTD device %s\n",
  		...
  		mtd_part_parser_put(parser);  /* 将分区信息 put 给模块，也就是 flash 驱动 */
  		/*
  		 * Stash the first error we see; only report it if no parser
  		 * succeeds
  		 */
...
​		控制器驱动和设备驱动通过 `bus-num` 进行匹配。



* Canopus 芯片只有一路 SPI，所以在设备树中只能看到一路 SPI，并且 SPI 的驱动和 Flash 的驱动是通过 bus-num 来进行匹配的
  ...
  * Flash 驱动描述：
    ...
    /* Linux 会依次匹配每一个 spi_board_info 结构，比较 spi->bus_num 和 spi_board_info->bus_num */
    int spi_register_board_info(struct spi_board_info const *info, unsigned n)
    {
    	// xxx
    	for (i = 0; i < n; i++, bi++, info++) {
...
- init
  ...
  - 使用 `postcore_initcall(spi_init)` 在启动 initcall 的时候调用

  - malloc 一个 buf，最大是 32字节

    - buf 用来干嘛？

  - 调用 `bus_register(&spi_bus_type)` 注册 spi 总线

    ```c
    struct bus_type spi_bus_type = {
        .name = "spi",
        .dev_groups = spi_dev_groups,
        .match = spi_match_device,    # 至关重要
        .uevent = spi_uevent,
    };
    
    /*
     如果设备和驱动匹配的话，会调用 .match 接口来匹配。
     什么时候发生的设备和驱动匹配？
     	设备添加时：当一个新的 spi 设备被添加到系统中时，例如通过设备树、ACPI 表或者手动创建，内核会尝试将该设备与已注册的驱动程序进行匹配。
     		设备被添加到系统中时，内核会向 SPI 总线注册这个设备(通常是通过 spi_register_device / spi_new_device 函数)
     		设备添加后，会自动调用 SPI 总线中的 match 函数
        驱动注册时：当一个新的 spi 驱动程序被加载到系统中时，例如通过内核模块加载，内核会尝试将该驱动与已经存在的 SPI 设备进行匹配。
        	驱动程序被加载时，会通过 `spi_register_driver` 函数注册到 SPI 总线。注册后，SPI 总线会遍历所有已经存在的 SPI 设备。
        如果驱动和设备匹配成功，就会调用驱动的 probe 函数对设备进行初始化。
    ...
    */
    ...
    - 申请一个私有数据 `struct subsys_provate *priv`

    - 配置这个私有数据，包括 kobj 相关的东西

    - 调用 `kset_register` 注册 kset

    - 调用 `bus_create_file` 生成 uevent 文件，后面可以对这个文件产生 uevent 事件

    - 调用 `kset_create_and_add` 生成 device、drivers

    - 将创建的 device、driver 初始化到一个链表
    ...
    - 创建完成后目录结构如下：
      ...
      ├── devices/          # 挂接在此总线上的设备
      ├── drivers/          # 与此总线相关的驱动程序
      └── uevent            # 总线的 uevent 文件
      
      
      # devices：在 /sys/bus/mybus/ 目录下，会有一个 devices/ 子目录，用于存放与该总线关联的所有驱动程序的符号链接。每当有设备被注册到这个总线上时，都会在这个目录下创建一个以符号链接指向设备的物理路径。
      # drivers：在 /sys/bus/mybus/ 目录下，会有一个 drivers/ 子目录，用于存放与该总线关联的所有驱动程序的符号链接。每个驱动程序都会在这个目录下创建一个已驱动名称命名的子目录。
      # uevent：这个文件用于向用户控件报告总线设备的热插拔事件。
      ```

      

  - 调用 `class_regiter(&spi_master_class)` 注册 spi_master 类
