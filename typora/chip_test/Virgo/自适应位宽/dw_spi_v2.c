	...
	*/
	...
	if (transfer->tx_buf) {
		...
		if ((transfer->bits_per_word == 0x20) && ((dws->cur_msg->spi->mode & SPI_TX_DUAL) | (dws->cur_msg->spi->mode & SPI_TX_QUAD))) {
			...
			dw_writel(dws, SPIM_TXDR_BE, data8[0]);
			dw_writel(dws, SPIM_TXDR_BE, ((data8[1] << 16) | (data8[2] << 8) | (data8[3] << 0)));
			data32++;
...
//				dw_writel(dws, SPIM_TXFTLR, 2<< 16);
//				dw_writel(dws, SPIM_RXFTLR, dws->rx_fifo_len - 1);
			...
			dw_writel(dws, SPIM_SPI_CTRLR0, 0x0);
//			dw_writel(dws, SPIM_TXFTLR, 0<< 16);
		}
...
//			items = dw_readl(dws, SPIM_TXFLR);
		...
		/* 5 个字节，一个32bit位宽发掉了，剩余的要 8bit 位宽发 */
		/* 1. align_steps = 0 的时候 */
		if (align_steps == 0) {
			spi_enable_chip(dws, 0);
			cr0 &= ~0x1f;
...
//	else
//		return dw_half_duplex_axidma_transfer(dws);
		return dw_half_duplex_poll_transfer(dws);
}
