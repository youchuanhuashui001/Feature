...
#if defined(CONFIG_ENABLE_GEN_SPI) && defined(CONFIG_ENABLE_GEN_SPI_TEST) && defined(AAA)
...
//		dw_writel(dws, SPIM_CTRLR1, steps-1);
		...
		while ((align_steps != 0) && (transfer->tx_nbits & (SPI_NBITS_DUAL | SPI_NBITS_QUAD | SPI_NBITS_OCTAL))) {
			wait_till_not_busy(dws);
